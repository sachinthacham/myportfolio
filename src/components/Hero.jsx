...
import image1 from '../Assets/Heroimages/new_image7.png';
import image2 from '../Assets/Heroimages/new_image7_opti.webp';
...
const Hero = () => {
  ...
  const socials = [
   ...
   // Set up inView hook to track visibility
   const [ref, inView] = useInView({
     threshold: 0.5, // Trigger when 50% of the element is in view
   });
 
   // Start the animation when the SVG is in view
   useEffect(() => {
     if (inView) {
       controls.start({
         scale: 1, // Full zoom in
         transition: { duration: 0.8, ease: 'easeOut' }
       });
     } else {
       controls.start({
         scale: 0.5 // Initial zoom out state
       });
     }
   }, [controls, inView]);
  ...
  return (
    ...
    <div className='h-full'>
      <div className='flex flex-col xl:flex-row items-center justify-between xl:pt-8 xl:pb-24'>
        {/* Left Section - Text */}
        <div className='text-center xl:text-left order-2 xl:order-none md:mt-7'>
          ...
          <h1 className='text-[32px] sm:text-[48px] xl:text-[60px] leading-[1.1] font-semibold mb-6'>
           <span className='text-[#ff014f]'>SACHINTHA </span>CHAMINDU<br />
            ...
            <span className='text-white xl:text-[40px] md:text-[30px] text-[20px]'>FULL STACK <span className=' font-[400]'>DEVELOPER</span></span>
  ...
  <FaFileDownload className='text-xl' />
        ...
        <div className='order-1 xl:order-none mb-8 xl:mb-0 w-[250px] h-[250px] sm:w-[400px] sm:h-[400px] xl:w-[498px] xl:h-[498px] grid place-items-center '>
          <div className="relative w-full max-w-[200px] sm:max-w-[300px] lg:max-w-[360px] xl:max-w-[415px] mx-auto mt-[-50px] ">
            <motion.div 
            ref={ref}
            animate={controls}
            initial={{ scale: 0.5 }}
            className="absolute top-[20px] sm:top-[40px]  right-[-50px] w-[150%] h-auto">
              <svg id="animated-div"  viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
